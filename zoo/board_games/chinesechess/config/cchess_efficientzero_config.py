...
观察空间：(72, 10, 9) = 18层 × 4历史帧
- 14层棋子（己方7层 + 对方7层）
- 4层特征（当前玩家、重复计数、步数、限着计数）

启动方式：
    单卡: python zoo/board_games/chinesechess/config/cchess_efficientzero_config.py
    多卡: torchrun --nproc_per_node=8 zoo/board_games/chinesechess/config/cchess_efficientzero_config.py
"""
...
cchess_efficientzero_config = dict(
    exp_name=f'data_efficientzero/cchess_efficientzero_ns{num_simulations}_seed0',
    env=dict(
    ...
    policy=dict(
        model=dict(
            model_type='conv',
            observation_shape=(72, 10, 9),  # 18层 × 4帧 = 72
            action_space_size=ACTION_SPACE_SIZE,
            image_channel=72,  # 与 observation_shape[0] 一致
            num_res_blocks=9,
            num_channels=128,
...
if __name__ == "__main__":
    """
    单卡训练:
        python zoo/board_games/chinesechess/config/cchess_efficientzero_config.py
    
    多卡 DDP 训练:
        torchrun --nproc_per_node=8 zoo/board_games/chinesechess/config/cchess_efficientzero_config.py
    """
