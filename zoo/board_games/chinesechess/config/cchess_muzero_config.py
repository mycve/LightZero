...
- 使用 torchrun 或 ding 的 DDPContext 启动
"""
...
n_episode = collector_env_num    # 每次collect收集的episode数（每张卡）

evaluator_env_num = 3
num_simulations = 50

# 多卡时batch_size可以加大（每卡batch_size，总batch = batch_size * gpu_num）
batch_size = 512
...
cchess_muzero_config = dict(
    exp_name=f'data_muzero/cchess_muzero_{gpu_num}gpu_actors{n_actors}_envs{envs_per_actor}_ns{num_simulations}_seed0',
    env=dict(
...
cchess_muzero_create_config = dict(
    ...
    # Actor已经是独立进程，内部env_manager用base更高效（避免进程嵌套）
...
if __name__ == "__main__":
    ...
    def _sanitize_log_buffer(data):
        """将numpy数组转换为Python标量/列表，避免DDP日志聚合错误"""
