"""
五子棋 MuZero 多Actor并行采集配置

使用MultiActorMuZeroCollector实现多Actor并行采集，提升GPU利用率。

关键配置:
- n_actors: Actor数量，建议设置为 CPU核心数/2 ~ CPU核心数
...
gomoku_muzero_multi_actor_config = dict(
    exp_name=f'data_muzero/gomoku_muzero_multi_actor_sp-mode_actors{n_actors}_envs{envs_per_actor}_rand{prob_random_action_in_bot}_ns{num_simulations}_upc{update_per_collect}_rer{reanalyze_ratio}_seed0',
    env=dict(
...
gomoku_muzero_multi_actor_config = EasyDict(gomoku_muzero_multi_actor_config)
main_config = gomoku_muzero_multi_actor_config

gomoku_muzero_multi_actor_create_config = dict(
    env=dict(
        type='gomoku',
...
gomoku_muzero_multi_actor_create_config = EasyDict(gomoku_muzero_multi_actor_create_config)
create_config = gomoku_muzero_multi_actor_create_config
